...
def get_db_url():
    ...
    if not database_url:
        raise ValueError(
            "Database url is missing. Please add Render Postgres Sql External Databasae url in .env"
        )
...
# def flight_agent(state: TravelState):
#     query = state["user_query"]
#     flight_data = avaition_mcp_call(query)
...
def weather_agent(state: TravelState):
    ...
    return {
        ...
        """,
        "messages": [
            AIMessage(content = "Weather informaton fetched")
        ]
    }
...
def final_agent(state: TravelState):
    ...
    5. Estimated Budget
    6. Final Reccomandations

    Important:
    - Be clear and pracical.
...
# function to run graph Fast api
